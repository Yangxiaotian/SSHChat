...
2026-07-01 Xiangqi session compatibility: fast-forwarded local branch to origin/master to pick up Xiangqi repetition-rule code, then fixed old active games that lack _xq_ply_log (or contain invalid legacy log entries) by adding XiangqiGame._ensure_compat_state, __setstate__ migration, server rebind migration, and regression tests. Verified full Python unittest discover, electron TypeScript, Vite build, and game panel QA.

2026-07-01 Xiangqi click no-response fix: diagnosed frontend double-flipping coordinates when a black-top/red-bottom Xiangqi board was already oriented by the server. Added Playwright regression showing the bad command /game move coord 10 6 10 5, fixed flip detection to trust explicit board orientation before viewer side fallback, and added backend regression that black-top coord 1 4 -> 1 5 is legal. Verified game panel QA, Xiangqi board tests, tsc, vite build, and full Python unittest.

2026-07-02: 修复象棋点击/落子最后一轮问题：前端黑方在上视角不再重复翻转坐标；XiangqiGame 对旧对局缺失/损坏 _xq_ply_log 做兼容；server 本地与联邦 /game move 入口在 try_move 前强制修复旧运行时对象。验证：py_compile、test_board_ai_and_ratings、全量 unittest、qa-game-panels-playwright、tsc、vite build 均通过。

2026-07-02 Xiangqi professional advisor upgrade: strengthened hidden Xiangqi assistant with tactical move ordering, mate/check priority, quiescence search for capture/check lines, piece-safety and exchange-risk evaluation, deeper time-bounded search budgets, opponent-response prediction, and stronger server-side Xiangqi hard AI (depth 4 / wider beam). Added regression that hard AI prefers winning a major rook capture. Verified: test_board_ai_and_ratings, full unittest, tsc, vite build, qa-game-panels-playwright all pass.

2026-07-02 Xiangqi advisor recommendation visibility fix: root search now seeds lightweight fallback suggestions before deep tactical search, merges fallback results when time budget aborts early, and UI shows an explicit empty-state instead of a blank recommendation row. Verified with TypeScript, Xiangqi/rating unittest target, Vite build, and game panel Playwright QA.

2026-07-02 Xiangqi advisor exchange-safety fix: added immediate recapture/static exchange evaluation to root fallback and tactical scoring so superficially profitable captures such as cannon-takes-horse followed by rook recapture are heavily downgraded unless they produce mate/checking compensation or real material gain. Verified with TypeScript, Vite build, and game panel Playwright QA.

2026-07-02 Xiangqi professional rules upgrade: mapped user chess principles into executable advisor scoring for opening development, middle-game initiative, exchange safety, palace pressure, kill-pattern pressure, endgame priorities, no-mate/no-blunder hard penalties, and opponent-response checks. Repaired XiangqiPanel UTF-8/string corruption by normalizing piece parsing, turn/seat parsing, board parsing, and advisor UI strings. Verified: tsc, vite build, board/rating unittest target, game panel Playwright QA.

2026-07-02 Xiangqi board drift fix: increased Xiangqi board inner padding from 20px to 28px, normalized piece button rendering (appearance/margin/padding/box-sizing/z-index), made board overflow visible, separated board line/river/piece z-index layers, and fixed the visible cancel-selection label. Verified with tsc, vite build, Xiangqi/rating unit target, game panel Playwright QA, and a DOM center-alignment check for 32 pieces.

2026-07-02 Xiangqi suggestion button fix: recommendation buttons now use the same canPlay condition as board clicks instead of requiring myTurn separately, fixing no-response cases when turn parsing is absent or resumed logs are partial. Verified by Playwright clicking 建议1 and observing /game move coord 10 2 8 3, plus tsc, vite build, Xiangqi/rating unit target, and full game panel QA.

2026-07-02 Xiangqi latest-board parsing fix: XiangqiPanel now collects all terminal board rows and renders the last 10 rows, preventing older board snapshots in the same workbench text from being parsed as the current board. Added targeted Playwright verification with an old initial board followed by a moved rook board; UI now renders the latest moved rook position. Verified with tsc, vite build, Xiangqi/rating unit target, and full game panel QA.

2026-07-03: 象棋助手 Pikafish 优化：默认强度提升到硬件自适应线程/Hash（本机约10线程/1024MB），前端思考时间提高到8.5-12s；Pikafish解析优先使用本机稳定的 avxvnni，AVX512 放后避免启动异常；UCI调用改为等待 uciok/readyok 后再搜索，减少超时回退；新增 pikafish.log 记录阶段、耗时、错误。已通过 tsc、游戏面板QA、Python规则测试，并完成免安装包打包。
