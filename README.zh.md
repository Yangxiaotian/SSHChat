...
> 界面默认语言为 **英文**，可用 `/lang zh` 切回中文（按昵称保存）。三国杀文案本迭代仍以中文为主。英文入门文档见 `docs/en/`。

---

## Update (2026-08 共享画板)

- **新增 `/canvas`（别名 `/board`）**：当前房间共享画板（Excalidraw），走和 `/sendfile` 同一套 HTTP(S) 网页。
  `/canvas` 当前房间，`/canvas 昵称` 与某人私密画板，`/canvas #房间` 指定房间。
- **密钥不在网址里**：每人独立网址 + 6 位密钥，打开网页后输入密钥即可一起画（Excalidraw 场景同步）。
- **关闭 / 新开**：发起人 `/canvas close` 关闭当前房间画板；房间已有画板时再发 `/canvas` 会重发邀请，`/canvas new` 强制新开一局。
- **客户端**：图形客户端（tk / Electron）会自动打开画板；终端请把网址复制到浏览器。默认约 4 小时过期。
- 无 Cloudflare 的节点可经联邦公网代理托管（规则与 `/sendfile` 相同）。详细用法：`/help` 或 `/canvas help`。
...
- 手机 **SSHChat App**（iOS / Android 原生客户端）用户请看 **`小白使用说明书-手机App.md`**。
...
4. 发普通文字就是聊天。常用命令：`/help`、`/names` 或 `/users`、`/rooms`、`/join`、`/switch`、`/msg`、`/sendfile`、`/canvas`（画板，别名 `/board`）、`/clock`（棋钟，可在 Kindle 打开）、`/leave`、`/pad`、`/poll`、`/later`、`/game`、`/news`、`/library`（简写 `/lib`）、`/dict`、`/clear`。终端客户端输入 `/` 后按 **Tab** 可补全命令（类似 Linux shell）。
...
6. 要一起画画就输 `/canvas`（当前房间）、`/canvas 昵称`（私密画板）或 `/canvas #房间`。同样会收到**网址和密钥各一行**（密钥不在网址里）：用浏览器打开网址、输入密钥即可共同绘画。图形客户端会自动弹出画板；终端请把网址复制到浏览器。发起人可用 `/canvas close` 关闭。
...
多台已部署的 SSHChat 可以组成一张聊天网：同名房间合并、跨服私聊/`/names`、跨服对局，以及跨服 `/sendfile` / `/canvas` 通知（文件和画板会话仍在发起方或联邦公网托管节点，需公网可达的文件地址）。
...
- **共享画板 `/canvas`**：与 `/sendfile` 同一套文件网页；每人独立网址+密钥。无 Cloudflare 时同样可把画板托管到联邦里带公网隧道的节点。
...
| `/canvas` | 当前房间共享画板（别名 `/board`）；每人独立网址+密钥，打开网页后一起画 |
...
| `/later 30m 文本` | 私人提醒：到期只发给你自己（重启后保留；联邦同名也会收到；也可用 `2h`/`1d`、`09:30`、`明天 09:00`） |
| `/later list` | 查看你的待提醒 |
| `/later cancel <编号>` | 取消一条提醒 |
| `/game help` | 查看房间小游戏用法 |
...
- **文件收发是另一条通道。** `/sendfile` 走的是独立的 HTTPS 网页端口（默认 `8443`），不是 SSH。文件在服务器上落盘保存到过期为止，**管理员能看到**。别用它传你不希望管理员看到的东西。不需要就用 `--no-file-transfer` 关掉。共享画板 `/canvas` 走同一端口。
