...
| `/game help` | 查看房间小游戏用法 |
| `/library` | 联邦并集书目（本机 + 对端 epub / txt / md / pdf）；每人自带书签，翻页自动保存 |
| `/lib` | `/library` 的简写（子命令相同，如 `/lib open 1`） |
| `/library open <序号\|文件名[@节点]>` | 打开图书（对端书按页拉取）；`next` / `prev` / `page` 翻页 |
...
联邦开启时，`/library` 显示**各节点书目并集**（对端书带 `@节点`）；打开对端书时只按页拉取正文，不在联邦链路上复制整本文件。同名书可用 `/library open 书名@节点` 消歧。

| 命令 | 说明 |
|------|------|
| `/library` 或 `/lib` | 联邦并集书目（含你的书签进度） |
| `/library open <序号\|文件名[@节点]>` | 打开图书（有书签则从书签继续） |
| `/library next` / `prev` | 翻页（自动存书签） |
| `/library page <页码>` | 跳到指定页 |
| `/library bookmarks` | 列出我的全部书签 |
| `/library reset <序号\|文件名[@节点]>` | 清除某本书的书签 |
