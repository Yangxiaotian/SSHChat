...
4. 发普通文字就是聊天。常用命令：`/help`、`/names` 或 `/users`、`/whois`、`/rooms`、`/join`、`/switch`、`/msg`、`/clear`。本项目**不提供文件传输**（避免强制命令 SSH 下路径不可用、服务端明文中继等问题）。
...
|------|------|
| `/help` | 完整命令说明（多行） |
| `/names` 或 `/users` | 当前房间昵称列表（不显示 IP） |
| `/whois` 或 `/whois 昵称` | 当前房间内用户的连接 IP；带昵称时只查该人（大小写不敏感） |
