...
4. 发普通文字就是聊天。常用命令：`/help`、`/names` 或 `/users`、`/rooms`、`/join`、`/switch`、`/msg`、`/leave`、`/game`、`/news`、`/library`（简写 `/lib`）、`/dict`、`/clear`。终端客户端输入 `/` 后按 **Tab** 可补全命令（类似 Linux shell）。本项目**不提供文件传输**（避免强制命令 SSH 下路径不可用、服务端明文中继等问题）。
